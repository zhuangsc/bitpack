...
def _tower_loss(images, labels, num_classes, scope, reuse_variables=None, bits_ph=[]):
  ...
  with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
    if FLAGS.use_bitpack is False:
      logits, norms = inception.inference_alexnet_v2(images, num_classes, for_training=True, scope=scope, bits_ph=bits_ph)
    else:
      logits, norms = inception.inference_alexnet_v2_bitpack(images, num_classes, for_training=True, scope=scope, bits_ph=bits_ph)
...
def train(dataset):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    bits_ph = []
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv1"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv2"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv3"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv4"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv5"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc6"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc7"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc71"))
    ...
    for step in range(FLAGS.max_steps):
      ...
      if FLAGS.profile is False:
        ...
        top_1 = 1
        top_5 = 2
        _, loss_value, norms = sess.run([train_op, loss, tower_norms], feed_dict=bits_dict, 
