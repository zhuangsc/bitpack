...
def inference_resnet_bitpack(inputs,
  ...
  with tf.variable_scope('', 'resnet', [inputs]) as sc:
    ...
    with tf.device("/cpu:0"):
      ...
      cpu_kernels1 = bit_packer.bit_pack_cpu_avx(kernels1, bits_ph[0])
      ...
      cpu_kernels2 = bit_packer.bit_pack_cpu_avx(kernels2, bits_ph[1])
      ...
      cpu_kernels3 = bit_packer.bit_pack_cpu_avx(kernels3, bits_ph[2])
      ...
      cpu_kernels4 = bit_packer.bit_pack_cpu_avx(kernels4, bits_ph[3])
...
class ResNet(object):
  ...
  def _bottleneck_residual(self, x, in_filter, out_filter, stride,
    ...
    with tf.variable_scope('sub1'):
      x = self._conv('conv1', x, 1, in_filter, out_filter/4, stride, var_list, scope)
  
    with tf.variable_scope('sub2'):
      x = self._batch_norm('bn2', x)
      x = self._relu(x, self.relu_leakiness)
      x = self._conv('conv2', x, 3, out_filter/4, out_filter/4, [1, 1, 1, 1], var_list, scope)
  ...
  def _bottleneck_residual_bitpack_kernels(self, in_filter, out_filter, stride,
   ...
   ## Get variables
    with tf.device("/cpu:0"):
      with tf.variable_scope('sub1'):
         k1, n1, s1 = self._conv_get_variable('conv1', 1, in_filter, out_filter/4, stride, var_list, scope)
         k1 = tf.reshape(k1, [n1])
  
      with tf.variable_scope('sub2'):
        k2, n2, s2 = self._conv_get_variable('conv2', 3, out_filter/4, out_filter/4, [1, 1, 1, 1], var_list,scope)
        k2 = tf.reshape(k2, [n2])
      ...
      with tf.variable_scope('sub_add'):
        if in_filter != out_filter:
          ...
          k4 = tf.reshape(k4, [n4])
          cpu_kernels = tf.concat([k1, k2, k3, k4], 0)
          return cpu_kernels, sum([n1, n2, n3, n4])
        else:
          cpu_kernels = tf.concat([k1, k2, k3], 0)
          return cpu_kernels, sum([n1, n2, n3])
  ...
  def _bottleneck_residual_bitpack(self, x, gpu_kernels, in_filter, out_filter, stride,
    ...
    n1, s1 = self._conv_get_variable_attr('conv1', 1, in_filter, out_filter/4, stride, var_list)
    n2, s2 = self._conv_get_variable_attr('conv2', 3, out_filter/4, out_filter/4, [1, 1, 1, 1], var_list)
    ...
    if in_filter != out_filter:
      gk1, gk2, gk3, gk4 = tf.split(gpu_kernels, [n1, n2, n3, n4])
      ...
      gk1, gk2, gk3 = tf.split(gpu_kernels, [n1, n2, n3])
      gk1 = tf.reshape(gk1, s1, name="GDW")
      gk2 = tf.reshape(gk2, s2, name="GDW")
      gk3 = tf.reshape(gk3, s3, name="GDW")
    ...
    with tf.variable_scope('sub1'):
      x = self._conv_bitpack('conv1', x, gk1, 1, in_filter, out_filter/4, stride, var_list)
  
    with tf.variable_scope('sub2'):
      x = self._batch_norm('bn2', x)
      x = self._relu(x, self.relu_leakiness)
      x = self._conv_bitpack('conv2', x, gk2, 3, out_filter/4, out_filter/4, [1, 1, 1, 1], var_list)
    ...
    with tf.variable_scope(name):
      ...
      with tf.device('/cpu:0'):
        ...
        if self.mode is True:
          ...
          tf.summary.histogram(mean.op.name, mean)
          tf.summary.histogram(variance.op.name, variance)
        #cmean = bit_packer.bit_pack_cpu_avx(mean, bits)
        #cvar = bit_packer.bit_pack_cpu_avx(variance, bits)

        # epsilon used to be 1e-5. Maybe 0.001 solves NaN problem in deeper net.
        #gbeta = bit_packer.bit_unpack_gpu(cbeta, bits, tf.size(beta), tf.shape(beta))
        #gbeta = tf.reshape(gbeta, tf.shape(beta))
        #ggamma = bit_packer.bit_unpack_gpu(cgamma, bits, tf.size(gamma), tf.shape(gamma))
        #ggamma = tf.reshape(ggamma, tf.shape(gamma))

        #gmean = bit_packer.bit_unpack_gpu(cmean, bits, tf.size(mean), tf.shape(mean))
        #gmean = tf.reshape(gmean, tf.shape(mean))
        #gvar = bit_packer.bit_unpack_gpu(cvar, bits, tf.size(variance), tf.shape(variance))
        #gvar = tf.reshape(gvar, tf.shape(variance))
