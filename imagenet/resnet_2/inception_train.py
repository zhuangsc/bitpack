...
def _tower_loss(images, labels, num_classes, scope, reuse_variables=None, bits_ph=[]):
  ...
  with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
    if FLAGS.use_bitpack is False:
      logits, norms = inception.inference_resnet(images, num_classes, 
              for_training=True, scope=scope, bits_ph=bits_ph)
    else:
      logits, norms = inception.inference_resnet_bitpack(images, num_classes, 
              for_training=True, scope=scope, bits_ph=bits_ph)
