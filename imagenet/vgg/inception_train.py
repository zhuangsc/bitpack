...
tf.app.flags.DEFINE_float('L2_reg', 0.0005, """L2 regularizaion""")
...
def _tower_loss(images, labels, num_classes, scope, reuse_variables=None, bits_ph=[]):
  ...
  with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):
    if FLAGS.use_bitpack is False:
      logits, norms = inception.inference_vgg_a(images, num_classes, for_training=True, scope=scope, bits_ph=bits_ph)
    else:
      logits, norms = inception.inference_vgg_a_bitpack(images, num_classes, for_training=True, scope=scope, bits_ph=bits_ph)

  # Build the portion of the Graph calculating the losses. Note that we will
  # assemble the total_loss using a custom function below.
  split_batch_size = images.get_shape().as_list()[0]
  losses = inception.loss_alex(logits, labels, batch_size=split_batch_size, scope=scope)
...
def train(dataset):
  ...
  with tf.Graph().as_default(), tf.device('/cpu:0'):
    ...
    bits_ph = []
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv10"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv11"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv20"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv21"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv30"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv31"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv32"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv33"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv40"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv41"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv42"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv43"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv50"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_conv51"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv52"))
    #bits_ph.append(tf.placeholder(tf.int32, name="bits_conv53"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc6"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc7"))
    bits_ph.append(tf.placeholder(tf.int32, name="bits_fc8"))
    ...
    #gweights = []
    #glayerinfo = []
    #wnp_name = 'weights_norm_{}_{}_{}_{}_{}_{}_{}.dat'.format(10, 2048, 0, FLAGS.digits, FLAGS.stride, FLAGS.interval, FLAGS.use_bitpack)
    #lip_name = 'layerinfo_{}_{}_{}_{}_{}_{}_{}.dat'.format(10, 2048, 0, FLAGS.digits, FLAGS.stride, FLAGS.interval, FLAGS.use_bitpack)
